...
Final screenshot PNGs may contain only `IHDR`, `IDAT`, and `IEND` chunks and
must be RGB or fully opaque RGBA. The workflow rejects text, EXIF, ICC,
timestamp, physical-dimension, APNG, private, and unknown chunks even when their
CRCs are valid. It caps each source PNG at 16 MiB, limits decompression to the
exact declared scanline size, consumes the complete IDAT stream, rejects
trailing compressed payloads, reconstructs every filtered scanline, and rejects
any non-opaque alpha value. This prevents ancillary, compressed, malformed, or
transparent-pixel payloads from carrying data that is invisible during image
...
The candidate input must be the complete directory generated by the Windows
Store candidate preflight: the staged `.msix` or `.exe`, staged third-party
notices, `candidate.json`, and the exact three-entry `SHA256SUMS.txt`, with no
additional files. The asset workflow validates the full schema-v3 evidence
shape, commit/tool/project bindings, legal-notices record, checksums, and the
route-specific signature contract. An EXE must carry valid timestamped
Authenticode evidence for the installer and installed PE payload. An MSIX may
record `pending-microsoft-store-signing` only when its Authenticode record is
`NotSigned`, its Partner Center identity and MakeAppx checks are present, and
its Store signature remains `not-issued`; this state is suitable for exact
candidate screenshots but is not publication evidence.
...
Initialization records only the candidate file name, size, SHA-256, route,
signature state, and candidate-evidence SHA-256. It creates the locale
directories but no placeholder PNGs. The script refuses to overwrite an
