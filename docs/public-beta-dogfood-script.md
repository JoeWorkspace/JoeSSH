...
This script is the repeatable operator dogfood path for `0.1.0-beta.20`.
...
Because `0.1.0-beta.20` is permanently source-only, those artifacts may be
published only under a later distinct unused version; they must never be added
to the beta.20 GitHub Release.
