...
const productionSources = import.meta.glob<string>(
  ["./**/*.{ts,tsx}", "!./**/*.test.{ts,tsx}", "!./vite-env.d.ts"],
  {
    eager: true,
    import: "default",
    query: "?raw",
...
const shellSourceExtensions = new Set([
  ".html",
  ".js",
  ".json",
  ".svg",
...
const staticFallbackShellContracts: StaticFallbackShellContract[] = [
  ...
  },
  {
    ...
    descriptionText:
      "JoeSSH Admin requires an internet connection. Please check your network and try again.",
...
const staticFallbackShellSourcePaths = new Set(
  staticFallbackShellContracts.map(({ sourcePath }) => sourcePath),
);
...
const staticFallbackIconStyleNeedles = [
  "font-size: 3rem;",
  "margin-bottom: 1rem;",
];
...
const staticFallbackDocumentShellPatterns = [
  ...
  },
  {
    ...
    pattern: /<meta name="theme-color" content="#101820" \/>/g,
  },
  {
    label: "color-scheme meta",
    pattern: /<meta name="color-scheme" content="dark light" \/>/g,
  },
  {
    label: "referrer meta",
    pattern:
      /<meta name="referrer" content="strict-origin-when-cross-origin" \/>/g,
  },
];
...
const staticAppShellDocumentPatterns = [
  ...
  { label: "body close", pattern: /<\/body>/g },
  { label: "html close", pattern: /<\/html>/g },
  { label: "charset meta", pattern: /<meta\s+charset="UTF-8"\s*\/>/g },
  {
    label: "viewport meta",
    pattern:
      /<meta\s+name="viewport"\s+content="width=device-width, initial-scale=1\.0"\s*\/>/g,
  },
  {
    label: "description meta",
    pattern:
      /<meta\s+name="description"\s+content="JoeSSH Admin - read-only team, device, role, and audit snapshots from a configured Sync service\."\s*\/>/g,
  },
  {
    label: "theme-color meta",
    pattern: /<meta\s+name="theme-color"\s+content="#101820"\s*\/>/g,
  },
  {
    label: "color-scheme meta",
    pattern: /<meta\s+name="color-scheme"\s+content="dark light"\s*\/>/g,
  },
  {
  ...
  },
  {
    ...
    pattern:
      /<meta\s+name="apple-mobile-web-app-status-bar-style"\s+content="black-translucent"\s*\/>/g,
  },
  {
    label: "content security policy meta",
    pattern:
      /<meta\s+http-equiv="Content-Security-Policy"\s+content="default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; font-src 'self'; worker-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; upgrade-insecure-requests"\s*\/>/g,
  },
  {
    label: "permissions policy meta",
    pattern:
      /<meta\s+http-equiv="Permissions-Policy"\s+content="camera=\(\), microphone=\(\), geolocation=\(\), payment=\(\), usb=\(\), magnetometer=\(\), gyroscope=\(\), accelerometer=\(\)"\s*\/>/g,
  },
  {
    label: "referrer meta",
    pattern:
      /<meta\s+name="referrer"\s+content="strict-origin-when-cross-origin"\s*\/>/g,
  },
  {
    label: "content type options meta",
    pattern:
      /<meta\s+http-equiv="X-Content-Type-Options"\s+content="nosniff"\s*\/>/g,
  },
  {
    label: "open graph title",
    pattern: /<meta\s+property="og:title"\s+content="JoeSSH Admin"\s*\/>/g,
  },
  {
    label: "open graph description",
    pattern:
      /<meta\s+property="og:description"\s+content="Read-only team, device, role, and audit snapshots from a configured JoeSSH Sync service\."\s*\/>/g,
  },
  {
    label: "open graph type",
    pattern: /<meta\s+property="og:type"\s+content="website"\s*\/>/g,
  },
  {
    label: "twitter card",
    pattern: /<meta\s+name="twitter:card"\s+content="summary"\s*\/>/g,
  },
  {
    label: "twitter title",
    pattern: /<meta\s+name="twitter:title"\s+content="JoeSSH Admin"\s*\/>/g,
  },
  {
    label: "twitter description",
    pattern:
      /<meta\s+name="twitter:description"\s+content="Read-only team, device, role, and audit snapshots from a configured JoeSSH Sync service\."\s*\/>/g,
  },
...
const staticNonServiceWorkerShellExpectedSourcePaths =
  staticShellExpectedSourcePaths.filter(
...
const staticServiceWorkerCachePolicyPatterns = [
  {
    label: "versioned cache namespace",
    pattern: /\bconst\s+CACHE_NAME\s*=\s*(['"])joessh-admin-v3\1\s*;/,
  },
  {
    label: "bounded cache entry count",
    pattern: /\bconst\s+MAX_CACHE_ENTRIES\s*=\s*100\s*;/,
  },
  {
    label: "exact static asset inventory",
    pattern:
      /\bconst\s+STATIC_ASSETS\s*=\s*\[\s*(['"])\/\1\s*,\s*(['"])\/index\.html\2\s*,\s*(['"])\/manifest\.json\3\s*,\s*(['"])\/favicon\.svg\4\s*,\s*(['"])\/offline\.html\5\s*,?\s*\]\s*;/,
  },
  {
    label: "install-time cache warmup and activation",
    pattern:
      /caches\s*\.\s*open\(\s*CACHE_NAME\s*\)[\s\S]{0,160}cache\s*\.\s*addAll\(\s*STATIC_ASSETS\s*\)[\s\S]{0,80}self\s*\.\s*skipWaiting\(\s*\)/,
  },
  {
    label: "stale cache eviction",
    pattern:
      /keys\s*\.\s*filter\(\s*\(\s*k\s*\)\s*=>\s*k\s*!==\s*CACHE_NAME\s*\)\s*\.\s*map\(\s*\(\s*k\s*\)\s*=>\s*caches\s*\.\s*delete\(\s*k\s*\)\s*\)/,
  },
  {
    label: "client claim after activation",
    pattern: /self\s*\.\s*clients\s*\.\s*claim\(\s*\)/,
  },
  {
    label: "GET-only request handling",
    pattern: /if\s*\(\s*request\.method\s*!==\s*(['"])GET\1\s*\)\s*\{/,
  },
  {
    label: "same-origin request boundary",
    pattern: /if\s*\(\s*url\.origin\s*!==\s*self\.location\.origin\s*\)\s*\{/,
  },
  {
    label: "API cache exclusion",
    pattern:
      /if\s*\(\s*url\.pathname\s*===\s*(['"])\/api\1\s*\|\|\s*url\.pathname\.startsWith\(\s*(['"])\/api\/\2\s*\)\s*\)\s*\{/,
  },
  {
    label: "manifest classification",
    pattern:
      /\bconst\s+isManifest\s*=\s*url\.pathname\s*===\s*(['"])\/manifest\.json\1\s*;/,
  },
  {
    label: "static asset and navigation eligibility boundary",
    pattern:
      /if\s*\(\s*!isStaticAsset\s*&&\s*!isNavigation\s*&&\s*!isManifest\s*\)\s*\{/,
  },
  {
    label: "response interception",
    pattern: /event\.respondWith\(/,
  },
  {
    label: "network-first navigation",
    pattern:
      /event\.respondWith\(\s*networkFirstNavigation\(request\)\s*\)\s*;/,
  },
  {
    label: "stale-while-revalidate static assets",
    pattern: /event\.respondWith\(\s*staleWhileRevalidate\(request\)\s*\)\s*;/,
  },
  {
    label: "stale-while-revalidate implementation",
    pattern: /async\s+function\s+staleWhileRevalidate\(\s*request\s*\)\s*\{/,
  },
  {
    label: "network-first implementation",
    pattern: /async\s+function\s+networkFirstNavigation\(\s*request\s*\)\s*\{/,
  },
  {
    label: "cacheability implementation",
    pattern: /function\s+isCacheableResponse\(\s*response\s*\)\s*\{/,
  },
  {
    label: "basic same-origin response requirement",
    pattern: /response\.type\s*===\s*(['"])basic\1/,
  },
  {
    label: "redirect exclusion",
    pattern: /!response\.redirected/,
  },
  {
    label: "private and no-store cache-control exclusion",
    pattern:
      /\/\(\?:\^\|,\)\\s\*\(\?:private\|no-store\)\\b\/i\.test\(\s*cacheControl\s*\)/,
  },
  {
    label: "offline response implementation",
    pattern: /function\s+offlineResponse\(\s*\)\s*\{/,
  },
  {
    label: "cache trimming implementation",
    pattern: /async\s+function\s+trimCache\(\s*cache\s*\)\s*\{/,
  },
  {
    label: "cache size boundary",
    pattern: /if\s*\(\s*keys\.length\s*>\s*MAX_CACHE_ENTRIES\s*\)\s*\{/,
  },
  {
    label: "oldest cache entry eviction",
    pattern: /await\s+cache\.delete\(\s*keys\[0\]\s*\)\s*;/,
  },
];
...
function collectShellSources(
  ...
  for (const entry of readdirSync(rootPath, { withFileTypes: true })) {
    ...
    if (
      shellSourceExtensions.has(extname(entry.name)) ||
      shellSourceExtensionlessFiles.has(entry.name)
    ) {
...
function objectPropertyPatterns(
  objectName: string,
  propertyName: string,
): RegExp[] {
  ...
  return [
    ...
    new RegExp(
      String.raw`\{[^}]*\b${escapedPropertyName}\b[^}]*\}\s*=\s*${objectReference}`,
    ),
    new RegExp(
      String.raw`\{[^}]*\b${escapedObjectName}\s*:\s*([A-Za-z_$][\w$]*)[^}]*\}\s*=\s*${platformObjectExpression}\s*;[\s\S]{0,1200}\b\1\s*${propertyAccess}`,
    ),
    new RegExp(
      String.raw`\b(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*${objectReference}\s*;[\s\S]{0,1200}\b\1\s*${propertyAccess}`,
    ),
    new RegExp(
...
function methodCallPatterns(methodName: string): RegExp[] {
  ...
  return [
    ...
    new RegExp(
      String.raw`${objectReference}\s*(?:\?\.)?\[\s*${methodExpression}\s*\]${methodCall}`,
    ),
    new RegExp(
...
function platformPropertyAccessPatterns(propertyName: string): RegExp[] {
  ...
  return [
    ...
    new RegExp(
      String.raw`\{[^}]*\b${escapedPropertyName}\b[^}]*\}\s*=\s*${platformReference}`,
    ),
  ];
}

function passiveResourceElementCreationPatterns(
  elementNames: string[],
): RegExp[] {
  ...
  const createElementExpression =
    stringLiteralExpressionPattern("createElement");
  ...
  return [
    new RegExp(
      String.raw`${documentExpression}\s*${createElementCall}${elementExpression}`,
    ),
    new RegExp(
...
function expectAnyPatternMatches(
  patterns: RegExp[],
  source: string,
  label: string,
) {
...
function findStaticShellSourceByteBudgetIssues(
  sourcePath: string,
  source: string,
) {
  ...
  return byteLength > byteBudget
    ? [`${byteLength} bytes exceeds ${byteBudget} byte budget`]
    : [];
}
...
function collectManifestAppIntegrationKeys(
  value: unknown,
...
function findManifestAppIntegrationKeys(
...
const nonServiceWorkerShellSources: Record<string, string> = Object.fromEntries(
  Object.entries(productionShellSources).filter(
    ([sourcePath]) => sourcePath !== "../public/sw.js",
  ),
);
...
const browserNetworkAllowedAccessPatterns = new Map<string, readonly RegExp[]>([
  [
    "./main.tsx",
    [/\bloadAdminDashboard\(\s*window\.fetch\.bind\(\s*window\s*\),/],
  ],
]);
...
describe("web admin accessibility contracts", () => {
  it("keeps production Web Admin sources off source-level aria-label attributes", () => {
    ...
    const disallowedScriptExecutionPatterns = ["eval(", "new Function"];
    const allowedStaticShellEntrypointScript =
    ...
    const disallowedInlineEventHandlerPatterns = [
      /<[a-z][a-z0-9:-]*(?:\s+[^>]*?)?\son[a-z][\w:-]*\s*=/i,
    ];
    const disallowedInlineStyleAttributePatterns = [
      /<[a-z][a-z0-9:-]*(?:\s+[^>]*?)?\sstyle\s*=/i,
    ];
    ...
    const disallowedFormSubmissionPatterns = [
      ...
      /\bonSubmit\b/,
      new RegExp(
        String.raw`\baddEventListener\(\s*${stringLiteralExpressionPattern("submit")}`,
      ),
    ...
    const disallowedPassiveResourcePatterns = [
      ...
      ...passiveResourceElementCreationPatterns([
        "audio",
        "embed",
        "iframe",
        "img",
    ...
    const deviceSensorComputedKeyHint =
      "(?:Accelerometer|Sensor|Device|Gamepad|Gyroscope|Magnetometer|Orientation|XR)";
    ...
    const disallowedDeviceSensorPatterns = [
      ...
      new RegExp(
        String.raw`\{[^}]*\b${deviceSensorConstructorNames}\b[^}]*\}\s*=\s*${platformLikeReference}`,
      ),
      new RegExp(
      ...
      new RegExp(
        String.raw`\{[^}]*\b${navigatorSensorAccessNames}\b[^}]*\}\s*=\s*${navigatorReference}`,
      ),
      new RegExp(
        String.raw`${screenReference}\s*${orientationAccess}\s*${orientationLockAccess}\s*\(`,
      ),
      new RegExp(
    ...
    const disallowedNativeDialogPatterns = [
      ...
      /\bprompt\s*\(/,
      new RegExp(
        String.raw`\baddEventListener\(\s*${stringLiteralExpressionPattern("beforeunload")}`,
      ),
    ...
    const disallowedObserverAndFrameSchedulerPatterns = [
      ...
      new RegExp(
        String.raw`\{[^}]*\b${observerAndSchedulerPropertyNames}\b[^}]*\}\s*=\s*${platformReference}`,
      ),
    ];
    ...
    const disallowedBrowserTimerPattern =
    ...
    expect(
      Object.keys(productionShellSources).sort(),
      "static shell source inventory",
    ).toEqual(staticShellExpectedSourcePaths);
    expect(
      Object.keys(nonServiceWorkerShellSources).sort(),
      "non-Service Worker static shell source inventory",
    ).toEqual(staticNonServiceWorkerShellExpectedSourcePaths);
    expect(
      Object.keys(staticShellSourceByteBudgets).sort(),
      "static shell byte budget inventory",
    ).toEqual(staticShellExpectedSourcePaths);
    for (const [sourcePath, source] of Object.entries(productionShellSources)) {
      expect(
        findStaticShellTextEncodingIssues(source),
        `${sourcePath} static shell text encoding hygiene`,
      ).toEqual([]);
      expect(
        findStaticShellTextFormatIssues(source),
        `${sourcePath} static shell text format hygiene`,
      ).toEqual([]);
      expect(
        findStaticShellSourceByteBudgetIssues(sourcePath, source),
        `${sourcePath} static shell byte budget`,
      ).toEqual([]);
      if (sourcePath.endsWith(".json")) {
        expect(
          findDuplicateJsonKeys(source),
          `${sourcePath} static JSON duplicate-key hygiene`,
        ).toEqual([]);
      }
    }
    const staticManifest = JSON.parse(
      productionShellSources["../public/manifest.json"],
    );
    for (const {
      actionHref,
      actionLinkSource,
      ...
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback shell coverage`,
      ).toBeDefined();
      ...
      expect(
        countPatternMatches(
          fallbackSource,
          new RegExp(
            String.raw`<title>${escapeRegExp(titleText)}</title>`,
            "g",
          ),
        ),
      ...
      expect(
        countPatternMatches(fallbackSource, /<main(?:\s|>)/g),
        `${fallbackSourcePath} fallback native main landmark`,
      ).toBe(1);
      expect(
        countPatternMatches(fallbackSource, /<\/main>/g),
        `${fallbackSourcePath} fallback native main landmark`,
      ).toBe(1);
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback source-level role guard`,
      ).not.toMatch(/\srole\s*=/);
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback unused button guard`,
      ).not.toMatch(/\bbutton\b/);
      expect(
        countPatternMatches(
          fallbackSource,
          new RegExp(
            String.raw`<div\s+class="icon"\s+aria-hidden="true">${escapeRegExp(iconText)}</div>`,
            "g",
          ),
        ),
      ...
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback focus-visible style`,
      ).toContain(":focus-visible");
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback focus outline style`,
      ).toContain("outline: 2px solid #58a6ff;");
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback focus outline offset style`,
      ).toContain("outline-offset: 2px;");
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback reduced-motion style`,
      ).toContain("@media (prefers-reduced-motion: reduce)");
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback reduced-motion transition guard`,
      ).toContain("transition: none;");
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback forced-colors style`,
      ).toContain("@media (forced-colors: active)");
      expect(
        fallbackSource,
        `${fallbackSourcePath} fallback forced-colors action border`,
      ).toContain("border: 1px solid ButtonText;");
      expect(
        countPatternMatches(fallbackSource, /<style>/g),
        `${fallbackSourcePath} fallback style block open`,
      ).toBe(1);
      expect(
        countPatternMatches(fallbackSource, /<\/style>/g),
        `${fallbackSourcePath} fallback style block close`,
      ).toBe(1);
      expect(
        countPatternMatches(
          fallbackSource,
          cssLinePattern(`${actionSelector} {`),
        ),
      ...
      expect(
        countPatternMatches(
          fallbackSource,
          cssLinePattern(
            `${cssStateSelector(actionSelector, ":hover")} { background: #2ea043; }`,
          ),
        ),
      ...
      expect(
        countPatternMatches(
          fallbackSource,
          cssLinePattern(
            `${cssStateSelector(actionSelector, ":focus-visible")} {`,
          ),
        ),
      ...
      expect(
        countPatternMatches(
          fallbackSource,
          cssLinePattern(`${actionSelector} { transition: none; }`),
        ),
      ...
      expect(
        countPatternMatches(
          fallbackSource,
          cssLinePattern(`${actionSelector} { border: 1px solid ButtonText; }`),
        ),
      ...
      for (const styleNeedle of staticFallbackActionStyleNeedles) {
        expect(
          fallbackSource,
          `${fallbackSourcePath} fallback primary action style`,
        ).toContain(styleNeedle);
      }
      for (const styleNeedle of staticFallbackPageFrameStyleNeedles) {
        expect(
          fallbackSource,
          `${fallbackSourcePath} fallback page frame style`,
        ).toContain(styleNeedle);
      }
      for (const styleNeedle of staticFallbackIconStyleNeedles) {
        expect(
          fallbackSource,
          `${fallbackSourcePath} fallback icon style`,
        ).toContain(styleNeedle);
      }
      expect(
        countPatternMatches(
          fallbackSource,
          new RegExp(String.raw`<h1>${escapeRegExp(headingText)}</h1>`, "g"),
        ),
      ...
      expect(
        countPatternMatches(
          fallbackSource,
          new RegExp(String.raw`<p>${escapeRegExp(descriptionText)}</p>`, "g"),
        ),
      ...
      if (statusText === undefined) {
        expect(
          fallbackSource,
          `${fallbackSourcePath} fallback status-code guard`,
        ).not.toContain('class="status-code"');
      } else {
        expect(
          countPatternMatches(
            fallbackSource,
            new RegExp(
              String.raw`<p\s+class="status-code">${escapeRegExp(statusText)}</p>`,
              "g",
            ),
          ),
        ...
        for (const styleNeedle of staticFallbackStatusCodeStyleNeedles) {
          expect(
            fallbackSource,
            `${fallbackSourcePath} fallback status-code style`,
          ).toContain(styleNeedle);
        }
      }
      expect(
        countPatternMatches(
          fallbackSource,
          new RegExp(
            String.raw`<a\b(?=[^>]*\shref="${escapeRegExp(actionHref)}")[^>]*>${escapeRegExp(actionText)}</a>`,
            "g",
          ),
        ),
      ...
      expect(
        countPatternMatches(
          fallbackSource,
          new RegExp(escapeRegExp(actionLinkSource), "g"),
        ),
    ...
    expect(findStaticShellTextEncodingIssues("hidden\u0000control")).toContain(
      "control U+0000",
    );
    expect(findStaticShellTextEncodingIssues("hidden\u009Fcontrol")).toContain(
      "control U+009F",
    ...
    expect(findStaticShellTextEncodingIssues("Icon 馃摗")).toContain(
      "mojibake emoji mojibake",
    );
    expect(findStaticShellTextFormatIssues("line\r\n")).toContain(
      "carriage return",
    );
    expect(findStaticShellTextFormatIssues("line")).toContain(
    ...
    const oversizedRobotsSource = "x".repeat(
      staticShellSourceByteBudgets["../public/robots.txt"] + 1,
    );
    expect(
      findStaticShellSourceByteBudgetIssues(
        "../public/robots.txt",
        oversizedRobotsSource,
      ),
    ).toContain("129 bytes exceeds 128 byte budget");
    expect(findDuplicateJsonKeys('{"id":"/","id":"/admin"}')).toContain("$.id");
    expect(
      findDuplicateJsonKeys('{"shortcuts":[{"name":"Team","name":"Shadow"}]}'),
    ).toContain("$.shortcuts[0].name");
    ...
    const staticServiceWorkerSource = productionShellSources[
      "../public/sw.js"
    ].replace(/\r\n/g, "\n");
    expect(
      Array.from(
        staticServiceWorkerSource.matchAll(
          /self\.addEventListener\(\s*(['"`])([^'"`]+)\1/g,
        ),
        (match) => match[2],
      ),
      "../public/sw.js static Service Worker event inventory",
    ).toEqual(staticServiceWorkerExpectedEventTypes);
    for (const { label, pattern } of staticServiceWorkerCachePolicyPatterns) {
      expect(
        staticServiceWorkerSource,
        `../public/sw.js static Service Worker cache policy: ${label}`,
      ).toMatch(pattern);
    }
    expect(
      countPatternMatches(
        staticServiceWorkerSource,
        /^const\s+CACHE_NAME\s*=\s*(['"])joessh-admin-v3\1\s*;$/gm,
      ),
      "../public/sw.js static Service Worker cache name cardinality",
    ).toBe(1);
    expect(staticServiceWorkerSource).not.toMatch(
      /\bCACHE_NAME\s*=\s*(['"])joessh-admin-v2\1/,
    );
    expect(
      countPatternMatches(
        staticServiceWorkerSource,
        /^const MAX_CACHE_ENTRIES = 100;$/gm,
      ),
    ...
    expect(
      countPatternMatches(
        staticServiceWorkerSource,
        /\bconst\s+STATIC_ASSETS\s*=\s*\[\s*(['"])\/\1\s*,\s*(['"])\/index\.html\2\s*,\s*(['"])\/manifest\.json\3\s*,\s*(['"])\/favicon\.svg\4\s*,\s*(['"])\/offline\.html\5\s*,?\s*\]\s*;/g,
      ),
      "../public/sw.js static Service Worker precache asset cardinality",
    ).toBe(1);
    expect(
      productionShellSources["../index.html"],
      "../index.html Vite entry script source",
    ).toContain(allowedStaticShellEntrypointScript);
    expectAnyPatternMatches(
    ...
    for (const manifestKey of [
      ...
      expect(
        findManifestAppIntegrationKeys(
          `{"${manifestKey}": []}`,
          disallowedManifestAppIntegrationKeys,
        ),
    ...
    expect(
      findManifestAppIntegrationKeys(
        '{"shortcuts":[{"share_target":{"action":"/share"}}]}',
        disallowedManifestAppIntegrationKeys,
      ),
    ...
    for (const [sourcePath, source] of Object.entries(productionSources)) {
      ...
      for (const disallowedPattern of disallowedHtmlInjectionPatterns) {
        expect(source, `${sourcePath} raw HTML injection guard`).not.toContain(
          disallowedPattern,
        );
      }
      for (const disallowedPattern of disallowedScriptExecutionPatterns) {
        expect(
          source,
          `${sourcePath} dynamic script execution guard`,
        ).not.toContain(disallowedPattern);
      }
      for (const disallowedPattern of disallowedUrlProtocolPatterns) {
        expect(source, `${sourcePath} unsafe URL protocol guard`).not.toContain(
          disallowedPattern,
        );
      }
      const allowedBrowserStoragePatterns =
        browserStorageAllowedAccessPatterns.get(sourcePath) ?? [];
      if (allowedBrowserStoragePatterns.length === 0) {
        for (const disallowedPattern of disallowedBrowserStoragePatterns) {
          expect(
            source,
            `${sourcePath} browser storage access guard`,
        ...
        for (const allowedBrowserStoragePattern of allowedBrowserStoragePatterns) {
          expect(
            source,
            `${sourcePath} audited browser storage access guard`,
          ).toMatch(allowedBrowserStoragePattern);
        }
        const sourceWithoutAllowedBrowserStorageAccess =
          allowedBrowserStoragePatterns.reduce(
            (candidateSource, allowedBrowserStoragePattern) =>
              candidateSource.replace(allowedBrowserStoragePattern, ""),
            source,
          );
        for (const disallowedPattern of disallowedBrowserStoragePatterns) {
          expect(
        ...
        expect(
          countPatternMatches(source, browserStorageAccessPattern),
          `${sourcePath} browser storage access guard`,
        ).toBe(allowedBrowserStoragePatterns.length);
      }
      const allowedBrowserNetworkPatterns =
        browserNetworkAllowedAccessPatterns.get(sourcePath) ?? [];
      if (allowedBrowserNetworkPatterns.length === 0) {
        for (const disallowedPattern of disallowedNetworkPatterns) {
          expect(
            source,
            `${sourcePath} browser network access guard`,
          ).not.toMatch(disallowedPattern);
        }
      } else {
        for (const allowedBrowserNetworkPattern of allowedBrowserNetworkPatterns) {
          expect(
            source,
            `${sourcePath} audited browser network access guard`,
          ).toMatch(allowedBrowserNetworkPattern);
        }
        const sourceWithoutAllowedBrowserNetworkAccess =
          allowedBrowserNetworkPatterns.reduce(
            (candidateSource, allowedBrowserNetworkPattern) =>
              candidateSource.replace(allowedBrowserNetworkPattern, ""),
            source,
          );
        for (const disallowedPattern of disallowedNetworkPatterns) {
          expect(
        ...
        expect(
          countPatternMatches(source, browserNetworkAccessPattern),
          `${sourcePath} browser network access guard`,
        ).toBe(allowedBrowserNetworkPatterns.length);
      }
      for (const disallowedPattern of disallowedRealtimeCommunicationPatterns) {
        expect(
          source,
          `${sourcePath} realtime browser communication guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedNavigationMutationPatterns) {
        expect(source, `${sourcePath} navigation mutation guard`).not.toMatch(
          disallowedPattern,
        );
      }
      for (const disallowedPattern of disallowedCrossWindowMessagingPatterns) {
        expect(
          source,
          `${sourcePath} cross-window messaging guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedFormSubmissionPatterns) {
        expect(source, `${sourcePath} form submission guard`).not.toMatch(
          disallowedPattern,
        );
      }
      ...
      for (const disallowedPattern of disallowedBrowserAppIntegrationPatterns) {
        expect(
          source,
          `${sourcePath} browser app integration guard`,
        ).not.toMatch(disallowedPattern);
      }
      const allowedServiceWorkerPatterns =
        serviceWorkerAllowedAccessPatterns.get(sourcePath) ?? [];
      if (allowedServiceWorkerPatterns.length === 0) {
        for (const disallowedPattern of disallowedServiceWorkerPatterns) {
          expect(
            source,
            `${sourcePath} service worker registration guard`,
          ).not.toMatch(disallowedPattern);
        }
      } else {
        for (const allowedServiceWorkerPattern of allowedServiceWorkerPatterns) {
          expect(
            source,
            `${sourcePath} audited service worker registration guard`,
          ).toMatch(allowedServiceWorkerPattern);
        }
        expect(
          countPatternMatches(source, serviceWorkerApiAccessPattern),
          `${sourcePath} service worker registration guard`,
        ).toBe(allowedServiceWorkerPatterns.length);
      }
      for (const disallowedPattern of disallowedDeviceSensorPatterns) {
        expect(source, `${sourcePath} device sensor API guard`).not.toMatch(
          disallowedPattern,
        );
      }
      for (const disallowedPattern of disallowedBrowserFingerprintingPatterns) {
        expect(
          source,
          `${sourcePath} browser fingerprinting guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedMediaCanvasCapturePatterns) {
        expect(
          source,
          `${sourcePath} media and canvas capture guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedDomExpansionPatterns) {
        expect(
          source,
          `${sourcePath} DOM expansion and credential guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedNativeDialogPatterns) {
        expect(source, `${sourcePath} native browser dialog guard`).not.toMatch(
          disallowedPattern,
        );
      }
      const allowedBrowserTimerPatterns =
        browserTimerAllowedCallPatterns.get(sourcePath) ?? [];
      if (allowedBrowserTimerPatterns.length === 0) {
        expect(source, `${sourcePath} browser timer source guard`).not.toMatch(
        ...
        for (const allowedTimerPattern of allowedBrowserTimerPatterns) {
          expect(
            source,
            `${sourcePath} audited browser timer source guard`,
          ).toMatch(allowedTimerPattern);
        }
        expect(
          countPatternMatches(source, browserTimerCallPattern),
          `${sourcePath} browser timer source guard`,
        ).toBe(allowedBrowserTimerPatterns.length);
      }
      for (const disallowedPattern of disallowedObserverAndFrameSchedulerPatterns) {
        expect(
          source,
          `${sourcePath} observer and frame scheduler guard`,
        ).not.toMatch(disallowedPattern);
      }
      expect(source, `${sourcePath} string timer guard`).not.toMatch(
        disallowedStringTimerPattern,
      );
    }
    for (const [sourcePath, source] of Object.entries(productionShellSources)) {
      expect(
        source,
        `${sourcePath} shell aria-label attribute guard`,
      ).not.toContain(disallowedAttribute);
      expect(
        source,
        `${sourcePath} shell redundant main role guard`,
      ).not.toContain('role="main"');
      ...
      if (sourcePath === "../index.html") {
        expect(
          countPatternMatches(
            source,
            new RegExp(escapeRegExp(allowedStaticShellEntrypointScript), "g"),
          ),
          `${sourcePath} shell Vite entry script guard`,
        ).toBe(1);
        for (const { label, pattern } of staticAppShellDocumentPatterns) {
          expect(
            countPatternMatches(source, pattern),
            `${sourcePath} app shell document ${label}`,
          ).toBe(1);
        }
      }
      const expectedLinkCount =
        sourcePath === "../index.html" ? allowedStaticShellLinkTags.length : 0;
      expect(
        countPatternMatches(source, staticShellLinkTagPattern),
        `${sourcePath} shell link tag guard`,
      ...
      if (sourcePath === "../index.html") {
        for (const allowedStaticShellLinkTag of allowedStaticShellLinkTags) {
          expect(
            countPatternMatches(
              source,
              new RegExp(escapeRegExp(allowedStaticShellLinkTag), "g"),
            ),
      ...
      if (sourcePath === "../index.html") {
        expect(
          countPatternMatches(
            source,
            new RegExp(escapeRegExp(allowedStaticShellRootMount), "g"),
          ),
          `${sourcePath} shell exact root mount guard`,
        ).toBe(1);
      }
      const expectedStyleBlockCount = staticFallbackShellSourcePaths.has(
        sourcePath,
      )
        ? 1
        : 0;
      ...
      for (const disallowedPattern of disallowedHtmlInjectionPatterns) {
        expect(
          source,
          `${sourcePath} shell raw HTML injection guard`,
        ).not.toContain(disallowedPattern);
      }
      for (const disallowedPattern of disallowedScriptExecutionPatterns) {
        expect(
          source,
          `${sourcePath} shell dynamic script execution guard`,
        ).not.toContain(disallowedPattern);
      }
      for (const disallowedPattern of disallowedInlineEventHandlerPatterns) {
        expect(
          source,
          `${sourcePath} shell inline event handler guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedInlineStyleAttributePatterns) {
        expect(
          source,
          `${sourcePath} shell inline style attribute guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedMetaRefreshPatterns) {
        expect(source, `${sourcePath} shell meta refresh guard`).not.toMatch(
          disallowedPattern,
        );
      }
      for (const disallowedPattern of disallowedBaseElementPatterns) {
        expect(source, `${sourcePath} shell base element guard`).not.toMatch(
          disallowedPattern,
        );
      }
      for (const disallowedPattern of disallowedFocusAttributePatterns) {
        expect(source, `${sourcePath} shell focus attribute guard`).not.toMatch(
          disallowedPattern,
        );
      }
      for (const disallowedPattern of disallowedInteractiveAttributePatterns) {
        expect(
          source,
          `${sourcePath} shell interactive attribute guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedUrlProtocolPatterns) {
        expect(
          source,
          `${sourcePath} shell unsafe URL protocol guard`,
        ).not.toContain(disallowedPattern);
      }
      for (const disallowedPattern of disallowedRealtimeCommunicationPatterns) {
        expect(
          source,
          `${sourcePath} shell realtime browser communication guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedNavigationMutationPatterns) {
        expect(
          source,
          `${sourcePath} shell navigation mutation guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedCrossWindowMessagingPatterns) {
        expect(
          source,
          `${sourcePath} shell cross-window messaging guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedFormSubmissionPatterns) {
        expect(source, `${sourcePath} shell form submission guard`).not.toMatch(
          disallowedPattern,
        );
      }
      for (const disallowedPattern of disallowedPassiveResourcePatterns) {
        expect(
          source,
          `${sourcePath} shell passive resource loading guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedFileTransferPatterns) {
        expect(
          source,
          `${sourcePath} shell clipboard and file transfer guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedPrivilegedBrowserApiPatterns) {
        expect(
          source,
          `${sourcePath} shell privileged browser API guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedContactPickerPatterns) {
        expect(source, `${sourcePath} shell contact picker guard`).not.toMatch(
          disallowedPattern,
        );
      }
      for (const disallowedPattern of disallowedProtocolHandlerPatterns) {
        expect(
          source,
          `${sourcePath} shell protocol handler guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedBrowserAppIntegrationPatterns) {
        expect(
          source,
          `${sourcePath} shell browser app integration guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedBrowserFingerprintingPatterns) {
        expect(
          source,
          `${sourcePath} shell browser fingerprinting guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedDeviceSensorPatterns) {
        expect(
          source,
          `${sourcePath} shell device sensor API guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedMediaCanvasCapturePatterns) {
        expect(
          source,
          `${sourcePath} shell media and canvas capture guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedDomExpansionPatterns) {
        expect(
          source,
          `${sourcePath} shell DOM expansion and credential guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedNativeDialogPatterns) {
        expect(
          source,
          `${sourcePath} shell native browser dialog guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedObserverAndFrameSchedulerPatterns) {
        expect(
          source,
          `${sourcePath} shell observer and frame scheduler guard`,
        ).not.toMatch(disallowedPattern);
      }
    }
    for (const [sourcePath, source] of Object.entries(
      nonServiceWorkerShellSources,
    )) {
      for (const disallowedPattern of disallowedBrowserStoragePatterns) {
        expect(
          source,
          `${sourcePath} non-service-worker shell browser storage guard`,
        ).not.toMatch(disallowedPattern);
      }
      for (const disallowedPattern of disallowedNetworkPatterns) {
        expect(
          source,
          `${sourcePath} non-service-worker shell browser network guard`,
        ).not.toMatch(disallowedPattern);
      }
      expect(
        source,
        `${sourcePath} non-service-worker shell browser timer guard`,
      ).not.toMatch(disallowedBrowserTimerPattern);
      expect(
        source,
        `${sourcePath} non-service-worker shell string timer guard`,
      ).not.toMatch(disallowedStringTimerPattern);
    }
    expect(
      findManifestAppIntegrationKeys(
        productionShellSources["../public/manifest.json"],
    ...
    expect(
      productionSources["./main.tsx"],
      "./main.tsx contract source",
    ).toContain("aria-labelledby=");
    expect(
      productionSources["./main.tsx"],
      "./main.tsx error-state source",
    ).toMatch(/message:\s*t\.local\((['"])web\.state\.error\.message\1\)/);
    expect(
      productionSources["./main.tsx"],
      "./main.tsx raw error detail source",
    ).not.toContain("adminError.message");
    expect(
      productionSources["./main.tsx"],
      "./main.tsx error-state source",
    ).not.toContain("state.message");
    expect(
      productionSources["./localization.ts"],
      "./localization.ts storage source",
    ).toContain("window.localStorage.setItem");
    expect(
      productionSources["./localization.ts"],
      "./localization.ts storage source",
    ).toContain("window.localStorage.getItem");
    expect(
      productionSources["./main.tsx"],
      "./main.tsx network source",
    ).toMatch(/loadAdminDashboard\(\s*window\.fetch\.bind\(window\)/);
  }, 30_000);
});
