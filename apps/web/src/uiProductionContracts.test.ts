...
describe("Web Admin production UI contracts", () => {
  ...
  it("makes the read-only and non-ready navigation boundaries explicit", () => {
    ...
    expect(mainSource).toMatch(/t\.local\(\s*(['"])web\.scope\.viewer\1\s*\)/);
    expect(mainSource).toMatch(/(['"])aria-disabled\1\s*:\s*true\s+as\s+const/);
    expect(mainSource).toMatch(
      /onClick\s*:\s*\(\s*event\s*:\s*React\.MouseEvent<HTMLAnchorElement>\s*\)\s*=>\s*event\.preventDefault\(\s*\)/,
    );
  ...
  it("renders localized section-level empty states instead of blank panels", () => {
    ...
    expect(mainSource).toMatch(
      /t\.local\(\s*(['"])web\.collection\.empty\1\s*\)/,
    );
  ...
  it("keeps narrow controls readable and preserves native select affordance", () => {
    expect(structuralStyles).toMatch(
      /\.languagePicker select\s*\{[\s\S]*?appearance:\s*auto\s*;/,
    );
    expect(themeStyles).toMatch(
      /@media \(max-width: 520px\)[\s\S]*?\.telemetryToggle\s*\{[\s\S]*?white-space:\s*normal\s*;/,
    );
    expect(themeStyles).toMatch(
      /\.telemetryToggle span\s*\{[\s\S]*?overflow-wrap:\s*anywhere\s*;/,
    );
  });

  it("allows long data panels to paginate while keeping print rows intact", () => {
    expect(themeStyles).toMatch(
      /@media print[\s\S]*?\.dashboardGrid,\s*\.panel\s*\{[\s\S]*?break-inside:\s*auto\s*;/,
    );
    expect(themeStyles).toMatch(
      /@media print[\s\S]*?\.memberTable \.row:not\(\.headerRow\),[\s\S]*?\.eventList li\s*\{[\s\S]*?break-inside:\s*avoid\s*;/,
    );
  });
  ...
  it("describes telemetry consent with the full privacy boundary", () => {
    ...
    expect(mainSource).toMatch(
      /t\.shared\(\s*(['"])desktop\.telemetryPrivacyHint\1\s*\)/,
    );
    expect(mainSource).toMatch(
      /t\.shared\(\s*(['"])web\.telemetryUnavailable\1\s*\)/,
    );
  });
});
