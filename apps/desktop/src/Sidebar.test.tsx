...
const conns: DesktopConnection[] = [
  { name: "prod-edge-01", host: "10.48.12.11", group: "Production", status: "sample", latencyLabelKey: "desktop.sampleDataShort", color: "neutral", tags: ["sample"] },
  { name: "eu-build-runner", host: "172.19.0.44", group: "CI runners", status: "busy", latencyMs: 72, color: "warn", tags: ["ci"], latencyHistory: [72] },
  { name: "db-replica-03", host: "db3.internal", group: "Data", status: "locked", latencyLabelKey: "desktop.mfaRequiredShort", color: "premium", tags: ["database"] },
...
function setup(overrides: Partial<React.ComponentProps<typeof Sidebar>> = {}) {
  ...
  const props: React.ComponentProps<typeof Sidebar> = {
    ...
    commandSnippets: [{ nameKey: "desktop.snippetPodStatus", command: "kubectl get pods" }],
    ...
    sidebarSearch: "",
    t,
    dragState: { order: conns.map((c) => c.name), dragging: null, dragOver: null },
    onDragStart: vi.fn(),
    onDragOver: vi.fn(),
    onDragLeave: vi.fn(),
    onDragEnd: vi.fn(),
...
describe("Sidebar", () => {
  ...
  it("activates a connection when its card is clicked", () => {
    const props = setup();
    fireEvent.click(screen.getByRole("button", { name: /prod-edge-01/i }));
    expect(props.onActivateConnection).toHaveBeenCalledWith("prod-edge-01");
  });

  it("marks only the active connection as current", () => {
    setup();
    expect(screen.getByRole("button", { name: /prod-edge-01/i }).getAttribute("aria-current")).toBe("true");
    expect(screen.getByRole("button", { name: /eu-build-runner/i }).hasAttribute("aria-current")).toBe(false);
  });

  it("labels status dots by connection status instead of color", () => {
    setup();

    expect(document.querySelector(".status-dot--neutral")?.getAttribute("aria-label")).toBe("desktop.sampleDataShort");
    expect(document.querySelector(".status-dot--warn")?.getAttribute("aria-label")).toBe("desktop.connectionBusy");
    expect(document.querySelector(".status-dot--premium")?.getAttribute("aria-label")).toBe("desktop.locked");
  ...
  it("collapses a group via its header", () => {
    const props = setup();
    fireEvent.click(screen.getByRole("button", { name: /desktop.groupProduction/ }));
    expect(props.onGroupDispatch).toHaveBeenCalledWith({ type: "TOGGLE_COLLAPSE", group: "Production" });
  });
  ...
  it("updates the sidebar search value", () => {
    const props = setup();
    fireEvent.change(screen.getByLabelText("desktop.searchPlaceholder"), { target: { value: "db" } });
    expect(props.onSidebarSearchChange).toHaveBeenCalledWith("db");
  });

  it("renders an empty state when no connections match", () => {
    setup({ filteredConnections: [], groupedConnections: new Map(), groupOrder: [] });
  ...
  it("opens the group manager and the command palette from the action bar", () => {
    ...
    expect(props.onGroupDispatch).toHaveBeenCalledWith({ type: "SET_MANAGER_OPEN", open: true });
  ...
  it("moves focused connections with Alt+Arrow keyboard shortcuts", () => {
    ...
    const props = setup({
      ...
      dragState: { order: prodConnections.map((c) => c.name), dragging: null, dragOver: null },
      favorites: [],
    });
    ...
    expect(props.onMoveConnectionBefore).toHaveBeenCalledWith("prod-edge-02", "prod-edge-01");

    fireEvent.keyDown(first, { key: "ArrowDown", altKey: true });
    expect(props.onMoveConnectionAfter).toHaveBeenCalledWith("prod-edge-01", "prod-edge-02");
  });
  ...
  it("does not move focused connections past group boundaries", () => {
    ...
    fireEvent.keyDown(screen.getByRole("button", { name: /prod-edge-01/i }), { key: "ArrowUp", altKey: true });
    fireEvent.keyDown(screen.getByRole("button", { name: /prod-edge-02/i }), { key: "ArrowDown", altKey: true });
  ...
  it("renders a custom group label verbatim when not in the builtin key map", () => {
    ...
    setup({ groupedConnections: custom, groupOrder: ["My Custom"], allGroupNames: ["My Custom"] });
    expect(screen.getByText("My Custom")).toBeTruthy();
  });

  it("renders custom latency labels verbatim when no latency key is provided", () => {
    const customLatency = [{ name: "db-replica-03", host: "db3.internal", group: "Data", status: "locked", latencyLabel: "External MFA", color: "premium", tags: ["database"] }] as unknown as DesktopConnection[];
    setup({
  ...
  it("renders translated latency keys through the provided translator", () => {
    const translated = ((key: string) => key === "desktop.mfaRequiredShort" ? "MFA required" : key) as any;
    setup({ t: translated });
    expect(screen.getByText("MFA required")).toBeTruthy();
  });

  it("prioritizes numeric latency over latency labels", () => {
    const withLatency = [{ name: "prod-edge-01", host: "10.48.12.11", group: "Production", status: "online", latencyMs: 12, latencyLabelKey: "desktop.mfaRequiredShort", color: "good", tags: [] }] as unknown as DesktopConnection[];
    setup({
  ...
  it("shows a localized latency fallback for a connection without latency data", () => {
    const noLatency = [{ name: "prod-edge-01", host: "10.48.12.11", group: "Production", status: "online", color: "good", tags: [] }] as unknown as DesktopConnection[];
    setup({
