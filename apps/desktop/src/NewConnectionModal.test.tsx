...
function setup(opts: { isNameAvailable?: (n: string) => boolean; onCreate?: (c: unknown) => boolean } = {}) {
  const onClose = vi.fn();
  const onCreate = vi.fn(opts.onCreate ?? (() => true));
  const isNameAvailable = vi.fn(opts.isNameAvailable ?? (() => true));
  const { container } = render(
    <NewConnectionModal defaultGroup="Production" isNameAvailable={isNameAvailable} onClose={onClose} onCreate={onCreate as never} t={t} />,
  );
...
describe("NewConnectionModal", () => {
  it("disables create until name and host are valid", () => {
    ...
    const button = screen.getByRole("button", { name: "desktop.createConnection" }) as HTMLButtonElement;
  ...
  it("creates a connection with parsed comma tags and closes", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "desktop.createConnection" }));

    await waitFor(() => expect(onClose).toHaveBeenCalled());
    expect(onCreate).toHaveBeenCalledWith({ name: "my-box", host: "10.0.0.1", group: "Production", tags: ["ssh", "gateway"] });
  });
  ...
  it("shows a name-taken error and keeps create disabled", () => {
    ...
    expect(alert.querySelector(".inline-alert-copy strong")?.textContent).toBe("desktop.nameTaken");
    expect(alert.querySelector(".inline-alert-copy small")).toBeNull();
    expect((screen.getByRole("button", { name: "desktop.createConnection" }) as HTMLButtonElement).disabled).toBe(true);
  });
  ...
  it("closes on Escape and falls back to defaultGroup when group cleared", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "desktop.createConnection" }));
    await waitFor(() => expect(onCreate).toHaveBeenCalledWith(expect.objectContaining({ group: "Production" })));
  ...
  it("edit mode prefills fields, locks the name, and saves an update ignoring availability", async () => {
    ...
    const { container } = render(
      <NewConnectionModal
        ...
        edit={{ name: "box", host: "old", group: "Personal", tags: ["ssh", "db"] }}
        onClose={onClose}
        onCreate={onCreate}
        t={t}
      />,
    ...
    await waitFor(() => expect(onClose).toHaveBeenCalled());
    expect(onCreate).toHaveBeenCalledWith({ name: "box", host: "new-host", group: "Personal", tags: ["ssh", "db"] });
  });
});
