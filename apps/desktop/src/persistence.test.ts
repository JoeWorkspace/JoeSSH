...
describe("desktop persistence helpers", () => {
  ...
  it("validates persisted layout values", () => {
    ...
    expect(
      readStoredLayout(defaultLayout, {
  ...
  it("sanitizes stored string lists", () => {
    ...
    expect(readStoredStringList(FAVORITES_KEY, { maxItems: 2 })).toEqual([
      "prod-edge-01",
      "staging-api",
    ]);
  });
  ...
  it("sanitizes persisted connection order and appends missing defaults", () => {
    ...
    ).toEqual(["staging-api", "prod-edge-01", "prod-edge-02"]);
  });
  ...
  it("caps custom connections and rejects blank or control-character fields", () => {
    installStorage({
      [CUSTOM_CONNECTIONS_STORAGE_KEY]: JSON.stringify([
        { name: "blank-host", host: " ", group: "g", tags: [] },
        { name: "bad\nname", host: "h", group: "g", tags: [] },
    ...
    expect(result[0]?.name).toBe("connection-0");
    expect(result[result.length - 1]?.name).toBe("connection-99");
  });
  ...
  it("returns an empty array for missing or non-array custom-connection storage", () => {
    ...
    installStorage({
      [CUSTOM_CONNECTIONS_STORAGE_KEY]: JSON.stringify({ not: "an array" }),
    });
    expect(readStoredCustomConnections()).toEqual([]);
  ...
  it("sanitizes persisted local forwarding rules and never restores live state", () => {
    installStorage({
      [FORWARD_RULES_STORAGE_KEY]: JSON.stringify([
        {
          active: true,
          bindHost: " 127.0.0.1 ",
          bindPort: 0,
    ...
    expect(readStoredForwardRules()).toEqual([
      {
        active: false,
        bindHost: "127.0.0.1",
        bindPort: 0,
