...
import { useConnectForm, validateConnectForm, type ConnectFormFields } from "./useConnectForm";
...
describe("validateConnectForm", () => {
  it("accepts a valid password form and trims fields", () => {
    const result = validateConnectForm({ ...VALID, host: "  h  ", username: "  u  " });
    expect(result).toEqual({
      ok: true,
      value: { host: "h", port: 22, username: "u", auth: { kind: "password", password: "secret" }, pinnedFingerprint: undefined },
    });
  });

  it("rejects empty host, bad port, empty username, empty password", () => {
    expect(validateConnectForm({ ...VALID, host: " " })).toMatchObject({ ok: false, reason: "host" });
    expect(validateConnectForm({ ...VALID, port: "0" })).toMatchObject({ ok: false, reason: "port" });
    expect(validateConnectForm({ ...VALID, port: "70000" })).toMatchObject({ ok: false, reason: "port" });
    expect(validateConnectForm({ ...VALID, port: "abc" })).toMatchObject({ ok: false, reason: "port" });
    expect(validateConnectForm({ ...VALID, username: "" })).toMatchObject({ ok: false, reason: "username" });
    expect(validateConnectForm({ ...VALID, password: "" })).toMatchObject({ ok: false, reason: "password" });
  });

  it("validates private-key auth and carries an optional passphrase + pinned fingerprint", () => {
    expect(validateConnectForm({ ...VALID, authKind: "private_key", pem: "  " })).toMatchObject({ ok: false, reason: "pem" });
    ...
    expect(result).toEqual({
      ok: true,
      value: { host: "example.com", port: 22, username: "lin", auth: { kind: "private_key", pem: "KEY", passphrase: "pp" }, pinnedFingerprint: "SHA256:abc" },
    });
  });

  it("omits an empty passphrase for private-key auth", () => {
    const result = validateConnectForm({ ...VALID, authKind: "private_key", pem: "KEY", passphrase: "" });
    expect(result).toMatchObject({ ok: true, value: { auth: { kind: "private_key", pem: "KEY", passphrase: undefined } } });
  });
});
...
describe("useConnectForm", () => {
  ...
  it("uses initial fields for validation and restores them on reset", async () => {
    ...
    const { result } = renderHook(() => useConnectForm(connect, { host: "server.internal", username: "atlas" }));
  ...
  it("connects successfully and exposes the session id", async () => {
    ...
    expect(result.current.status).toEqual({ phase: "connected", sessionId: "sess-1" });
    expect(connect).toHaveBeenCalledWith(expect.objectContaining({ host: "h", username: "u", port: 22 }));
  });
  ...
  it("captures connect errors as an error status", async () => {
    ...
    expect(result.current.status).toEqual({ phase: "error", message: "auth failed" });
  });
  ...
  it("clears an error status on the next field edit, and reset restores defaults", async () => {
    const { result } = renderHook(() => useConnectForm(vi.fn()));
    await act(async () => { await result.current.submit(); }); // -> error
