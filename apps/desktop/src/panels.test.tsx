...
const messages: Partial<Record<TranslationKey, string>> = {
  ...
  'desktop.forwardNoRulesHint': 'Add a rule to tunnel a local TCP port over SSH.',
  ...
  'desktop.sftpOverwriteDetail': 'A file named {name} already exists in this folder.',
  ...
  'desktop.sftpUp': 'Up',
  'desktop.knownHosts': 'Known hosts',  'desktop.knownHostsCount': '{count} stored host keys',
...
const activeConnection: ComponentProps<typeof InspectorPanel>['activeConnection'] = {
  color: 'good',
  group: 'Production',
  host: '10.48.12.11',
...
const connectionWithoutLatency: ComponentProps<typeof InspectorPanel>['activeConnection'] = {
  color: 'good',
  group: 'Staging',
  host: '10.48.12.99',
...
const inspectorConnectionStats: ComponentProps<typeof InspectorPanel>['connectionStats'] = {
  averageLatencyMs: 37,
  onlineConnections: 6,
  totalConnections: 8,
};
const inspectorSessionContext: ComponentProps<typeof InspectorPanel>['sessionContext'] = {
  regionLabel: 'us-east / edge',
  userHandle: 'lin.chen',
};

function renderInspectorPanel(activeConnectionOverride = activeConnection, hasActiveSession = true) {
  return (
    <InspectorPanel
...
describe('panel charts', () => {
  it('renders sparkline coordinates for empty and single-point series without NaN', () => {
    const emptyHtml = renderToStaticMarkup(<Sparkline color="good" values={[]} />);
    const singleHtml = renderToStaticMarkup(<Sparkline color="warn" values={[42]} />);
  ...
  it('renders accessible latency charts for empty and single-point series without NaN', () => {
    const emptyHtml = renderToStaticMarkup(<LatencyChart color="info" label="Latency chart, average 0 ms" values={[]} />);
    const singleHtml = renderToStaticMarkup(<LatencyChart color="good" label="Latency chart, average 42 ms" values={[42]} />);
...
describe('extracted desktop panels', () => {
  ...
  it('uses no-session inspector copy and disables remote runbook actions when disconnected', () => {
    vi.useFakeTimers();
    try {
      const { container } = render(renderInspectorPanel(activeConnection, false));
      act(() => { vi.advanceTimersByTime(700); });
      expect(container.innerHTML).toContain('No SSH session');
      expect(container.innerHTML).toContain('Connect before using remote actions.');
      expect(container.innerHTML).not.toContain('trusted');
      expect(container.innerHTML).not.toContain('Production SSH with recording');
      for (const button of container.querySelectorAll('.runbook-item button')) {
        expect((button as HTMLButtonElement).disabled).toBe(true);
        expect((button as HTMLButtonElement).title).toBe('Connect before using remote actions.');
      }
    } finally {
      vi.useRealTimers();
    }
  ...
  it('renders without latency metrics when connection lacks latencyMs and latencyHistory', () => {
    vi.useFakeTimers();
    try {
      const { container } = render(renderInspectorPanel(connectionWithoutLatency));
      act(() => { vi.advanceTimersByTime(700); });
      const html = container.innerHTML;
      expect(html).not.toContain('skeleton--card');
  ...
  it('does not leak NaN/Infinity for an empty latencyHistory', () => {
    vi.useFakeTimers();
    try {
      const emptyHistory = { ...activeConnection, latencyHistory: [] as number[] };
  ...
  it('renders SFTP file rows with localized file size and relative time', () => {
    const html = renderToStaticMarkup(<SftpPanel formatters={formatters} sftpItems={sftpItems} t={t} />);
  ...
  it('disables static SFTP fixture actions without a live transfer session', () => {
    const { container } = render(<SftpPanel formatters={formatters} sftpItems={sftpItems} t={t} />);
    ...
    expect((toolbarButtons[0] as HTMLButtonElement).title).toBe('Connect before using remote actions.');
    ...
    const html = renderToStaticMarkup(
      <SftpPanel
        ...
        directory={{
          active: true,
          path: '/var/remote',
          status: { phase: 'ready', entries: [{ name: 'remote.conf', is_dir: false, size: 2048 }, { name: 'data', is_dir: true, size: null }] },
        }}
      />,
    );
  ...
  it('renders the live SFTP loading state', () => {
    const html = renderToStaticMarkup(
      <SftpPanel formatters={formatters} sftpItems={sftpItems} t={t} directory={{ active: true, path: '/p', status: { phase: 'loading' } }} />,
    );
  ...
  it('renders the live SFTP error state', () => {
    const html = renderToStaticMarkup(
      <SftpPanel formatters={formatters} sftpItems={sftpItems} t={t} directory={{ active: true, path: '/p', status: { phase: 'error', message: 'denied' } }} />,
    );
  ...
  it('keeps the static SFTP fallback when the directory is inactive', () => {
    const html = renderToStaticMarkup(
      <SftpPanel formatters={formatters} sftpItems={sftpItems} t={t} directory={{ active: false, path: '/x', status: { phase: 'idle' } }} />,
    );
    expect(html).toContain('session.log');
    expect(html).toContain('/srv/atlas');
  });
  ...
  it('wires SFTP upload, file selection, toolbar download, and transfer states', () => {
    ...
    const { container, rerender } = render(
      <SftpPanel
        ...
        sftpItems={sftpItems}
        t={t}
        directory={{ active: true, path: '/srv', status: { phase: 'ready', entries: [{ name: 'app.log', is_dir: false, size: 12 }, { name: 'sub', is_dir: true, size: null }] }, onOpenDir, onGoUp, canGoUp: true }}
    ...
    expect((entries[0] as HTMLButtonElement).className).toContain('is-selected');
    ...
    const upBtn = Array.from(container.querySelectorAll('.sftp-toolbar button')).find((b) => b.textContent?.trim() === 'Up') as HTMLButtonElement;
    upBtn.click();
    expect(onGoUp).toHaveBeenCalled();

    rerender(
      <SftpPanel formatters={formatters} sftpItems={sftpItems} t={t}
        directory={{ active: true, path: '/srv', status: { phase: 'ready', entries: [] } }}
        transfer={{ status: { phase: 'transferring' }, onUpload, onDownload }} />,
    );
    expect(container.querySelector('.sftp-state--transfer')?.textContent).toContain('Transferring...');

    rerender(
      <SftpPanel formatters={formatters} sftpItems={sftpItems} t={t}
        directory={{ active: true, path: '/srv', status: { phase: 'ready', entries: [] } }}
        transfer={{ status: { phase: 'error', message: 'quota' }, onUpload, onDownload }} />,
    );
    expect(container.querySelector('.sftp-state--error')?.textContent).toContain('Transfer failed');
    expect(container.querySelector('.sftp-state--error')?.textContent).toContain('quota');
    expect(container.querySelector('.sftp-state--error')?.getAttribute('role')).toBe('alert');
  });
  ...
  it('disables unsafe live SFTP entry names before navigation or download', () => {
    ...
    const { container } = render(
      <SftpPanel
        ...
        sftpItems={sftpItems}
        t={t}
        directory={{ active: true, path: '/srv', status: { phase: 'ready', entries: [{ name: '../secret', is_dir: false, size: 12 }, { name: 'sub/dir', is_dir: true, size: null }, { name: 'safe.log', is_dir: false, size: 6 }] }, onOpenDir, canGoUp: true }}
        transfer={{ status: { phase: 'idle' }, onUpload, onDownload }}
      />,
    );
  ...
  it('requires confirmation before overwriting an existing SFTP file', () => {
    ...
    const firstAttempt = new File(['replace'], 'app.log', { type: 'text/plain' });
    fireEvent.change(input, { target: { files: [firstAttempt] } });

    expect(onUpload).not.toHaveBeenCalled();
    expect(container.querySelector('.sftp-overwrite-confirm')?.textContent).toContain('Replace existing file?');
    expect(container.querySelector('.sftp-overwrite-confirm')?.textContent).toContain('A file named app.log already exists in this folder.');

    fireEvent.click(within(container.querySelector('.sftp-overwrite-confirm') as HTMLElement).getByRole('button', { name: 'Cancel' }));
    expect(onUpload).not.toHaveBeenCalled();
    expect(container.querySelector('.sftp-overwrite-confirm')).toBeNull();

    const confirmedAttempt = new File(['replace'], 'app.log', { type: 'text/plain' });
  ...
  it('renders team access summary and review controls', () => {
    const html = renderToStaticMarkup(<TeamAccessPanel formatters={formatters} t={t} />);
  ...
  it('renders telemetry consent control and wires changes', () => {
    ...
    const { container, rerender } = render(
      <SettingsPanel t={t} telemetry={{ available: true, enabled: false, onChange }} />,
    );
    ...
    rerender(<SettingsPanel t={t} telemetry={{ available: false, enabled: false, onChange }} />);
    const disabledCheckbox = Array.from(container.querySelectorAll('input[type="checkbox"]'))
      .find((input) => input.closest('label')?.textContent?.includes('Error telemetry')) as HTMLInputElement;
  ...
  it('renders latency chart with gradient and data points', () => {
    const html = renderToStaticMarkup(
      <LatencyChart color="good" label="Latency chart" values={[32, 29, 35, 27, 30]} />,
    );
  ...
  it('renders latency chart with all color variants', () => {
    for (const color of ['neutral', 'good', 'warn', 'info', 'premium'] as const) {
      const html = renderToStaticMarkup(<LatencyChart color={color} label={`Chart ${color}`} values={[10, 20, 30]} />);
  ...
  it('disables fixture forwarding controls without live forward handlers', () => {
    ...
    for (const button of container.querySelectorAll('.forward-rule-actions button')) {
      expect((button as HTMLButtonElement).disabled).toBe(true);
      expect((button as HTMLButtonElement).title).toBe('Connect before using remote actions.');
    }
  });
  ...
  it('renders an emotional empty state for live SFTP directories with no entries', () => {
    const html = renderToStaticMarkup(
      <SftpPanel
        ...
        sftpItems={sftpItems}
        t={t}
        directory={{ active: true, path: '/empty', status: { phase: 'ready', entries: [] } }}
      />,
    );
  ...
  it('shows live forward runtime (bound address + active) and an error, and wires start/stop', () => {
    const rules = [
      { id: "fwd-a", direction: "Local" as const, bindHost: "127.0.0.1", bindPort: 5432, targetHost: "db", targetPort: 5432, active: false },
      { id: "fwd-b", direction: "Local" as const, bindHost: "127.0.0.1", bindPort: 6379, targetHost: "cache", targetPort: 6379, active: false },
    ];
    const onStart = vi.fn();
    const onStop = vi.fn();
    const forwards = {
    ...
    const { container } = render(<ForwardingPanel t={t} rules={rules} forwards={forwards} />);
    ...
    expect(alert?.querySelector('.inline-alert-copy strong')?.textContent).toBe("bind denied");
    expect(alert?.querySelector('.inline-alert-copy small')).toBeNull();

    const actionButtons = container.querySelectorAll('.forward-rule-actions button');
    ...
    expect(onStart).toHaveBeenCalledWith("fwd-b", "127.0.0.1:6379", "cache", 6379);
  });
      ...
      <ForwardingPanel
        ...
        forwards={{
          runtime: {
            ...
            "fwd-b": { active: true, pending: true, boundAddr: "127.0.0.1:55001" },
          },
          onStart,
          onStop,
        }}
  ...
  it('creates a local forwarding rule from the add form and starts it', () => {
    ...
    const onStop = vi.fn();
    const { container } = render(
      <ForwardingPanel t={t} rules={[]} forwards={{ runtime: {}, onStart, onStop }} />,
    );
    const view = within(container);

    fireEvent.click(view.getByRole('button', { name: 'Add forward rule' }));

    expect((view.getByLabelText('Local Host') as HTMLInputElement).value).toBe('127.0.0.1');
    expect((view.getByLabelText('Local Port') as HTMLInputElement).value).toBe('0');
    expect((view.getByLabelText('Remote Port') as HTMLInputElement).value).toBe('5432');

    fireEvent.change(view.getByLabelText('Remote Host'), { target: { value: 'db.internal' } });
    fireEvent.change(view.getByLabelText('Remote Port'), { target: { value: '5432' } });
    fireEvent.click(container.querySelector('.forward-rule-form-actions button[type="submit"]') as HTMLButtonElement);

    expect(container.innerHTML).toContain('127.0.0.1:0');
    expect(container.innerHTML).toContain('db.internal:5432');

    const startButton = view.getAllByRole('button', { name: 'Start' })[0] as HTMLButtonElement;
    fireEvent.click(startButton);
    expect(onStart).toHaveBeenCalledWith('custom-fwd-1', '127.0.0.1:0', 'db.internal', 5432);
  ...
  it('rejects unsafe forwarding bind hosts', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: 'Add forward rule' }));
    fireEvent.change(view.getByLabelText('Local Host'), { target: { value: '0.0.0.0' } });
    fireEvent.change(view.getByLabelText('Remote Host'), { target: { value: 'db.internal' } });

    const submitButton = container.querySelector('.forward-rule-form-actions button[type="submit"]') as HTMLButtonElement;
  ...
  it('formats IPv6 loopback bind addresses for created forwarding rules', () => {
    ...
    fireEvent.click(view.getByRole('button', { name: 'Add forward rule' }));
    fireEvent.change(view.getByLabelText('Local Host'), { target: { value: '::1' } });
    fireEvent.change(view.getByLabelText('Remote Host'), { target: { value: 'db.internal' } });
    fireEvent.click(container.querySelector('.forward-rule-form-actions button[type="submit"]') as HTMLButtonElement);

    fireEvent.click(view.getAllByRole('button', { name: 'Start' })[0] as HTMLButtonElement);
    expect(onStart).toHaveBeenCalledWith('custom-fwd-1', '[::1]:0', 'db.internal', 5432);
  });
  ...
  it('exports the provided custom connections as a JSON download', () => {
    ...
    URL.createObjectURL = () => { created(); return 'blob:x'; };
    URL.revokeObjectURL = vi.fn();
    const clickSpy = vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(function (this: HTMLAnchorElement) {
      downloadedFilename = this.download;
    });
    try {
      const conns = [{ name: 'box', host: '10.0.0.1', group: 'Personal', tags: ['ssh'] }];
      const { container } = render(<SettingsPanel t={t} connectionsIO={{ exportConnections: conns, onImport: vi.fn() }} />);
      const exportBtn = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.includes('Export connections')) as HTMLButtonElement;
      exportBtn.click();
  ...
  it('shows the stored known-host count and confirms before clearing them', async () => {
    ...
    const { container, rerender } = render(<SettingsPanel t={t} knownHosts={{ count: 3, entries: [], onClear, onRemove }} />);
    expect(container.innerHTML).toContain('3 stored host keys');
    const clearBtn = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.includes('Clear known hosts')) as HTMLButtonElement;
    expect(clearBtn.disabled).toBe(false);
    act(() => {
      clearBtn.click();
    });
    expect(onClear).not.toHaveBeenCalled();
    expect(container.innerHTML).toContain('Confirm clearing all stored host keys');
    ...
    rerender(<SettingsPanel t={t} knownHosts={{ count: 0, entries: [], onClear, onRemove }} />);
    const emptyBtn = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.includes('Clear known hosts')) as HTMLButtonElement;
  ...
  it('lists known-host pins with audit metadata and confirms before removing one pin', async () => {
    ...
    const removeBtn = Array.from(container.querySelectorAll('button')).find((b) => b.textContent?.includes('Remove host key')) as HTMLButtonElement;
    act(() => {
      removeBtn.click();
    });
