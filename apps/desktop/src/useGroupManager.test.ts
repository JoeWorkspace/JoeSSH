      ...
      try {
        ...
        return Array.isArray(arr)
          ? arr
              ...
              .slice(0, opts?.maxItems ?? Number.MAX_SAFE_INTEGER)
          : [];
      } catch {
        return [];
      }
    },
    readStoredConnectionGroups: (_opts: {
...
describe("useGroupManager", () => {
  ...
  it("initializes with builtin groups only", () => {
    ...
    expect(result.current.allGroupNames).toEqual([
      "Development",
      "Production",
      "Staging",
    ]);
  ...
  it("adds a custom group and persists it", () => {
    const { result } = renderHook(() => useGroupManager(BUILTIN, []));
    act(() =>
      result.current.dispatch({ type: "ADD_CUSTOM_GROUP", name: "NewGroup" }),
    );
    expect(result.current.state.customGroups).toContain("NewGroup");
    expect(result.current.allGroupNames).toContain("NewGroup");
    expect(JSON.parse(store.get("atlasterm.customGroups") ?? "[]")).toContain(
      "NewGroup",
    );
  });

  it("normalizes and de-duplicates custom groups in the reducer", () => {
    const { result } = renderHook(() => useGroupManager(BUILTIN, []));
    act(() =>
  ...
  it("removes a custom group and reassigns connections to Production", () => {
    ...
    act(() =>
      result.current.dispatch({ type: "REMOVE_CUSTOM_GROUP", name: "Old" }),
    );
  ...
  it("removes a custom group with no assigned connections", () => {
    const { result } = renderHook(() => useGroupManager(BUILTIN, []));
    act(() =>
      result.current.dispatch({ type: "ADD_CUSTOM_GROUP", name: "Empty" }),
    );
    act(() =>
      result.current.dispatch({ type: "REMOVE_CUSTOM_GROUP", name: "Empty" }),
    );
  ...
  it("renames a custom group and updates connection assignments", () => {
    ...
    act(() =>
      result.current.dispatch({ type: "ADD_CUSTOM_GROUP", name: "Other" }),
    );
    act(() =>
      result.current.dispatch({
        type: "MOVE_CONNECTION",
  ...
  it("does not rename onto an existing custom group (no duplicate group entry)", () => {
    ...
    expect(new Set(result.current.state.customGroups).size).toBe(
      result.current.state.customGroups.length,
    );
  });
  ...
  it("toggles collapse", () => {
    const { result } = renderHook(() => useGroupManager(BUILTIN, []));
    act(() =>
      result.current.dispatch({ type: "TOGGLE_COLLAPSE", group: "Production" }),
    );
  ...
  it("sets manager open state", () => {
    const { result } = renderHook(() => useGroupManager(BUILTIN, []));
    act(() =>
      result.current.dispatch({ type: "SET_MANAGER_OPEN", open: true }),
    );
    expect(result.current.state.managerOpen).toBe(true);
    act(() =>
      result.current.dispatch({ type: "SET_MANAGER_OPEN", open: false }),
    );
  ...
  it("handles editing flow", () => {
    ...
    act(() =>
      result.current.dispatch({ type: "SET_EDITING_GROUP_NAME", name: "Prod" }),
    );
  ...
  it("sets new group name", () => {
    const { result } = renderHook(() => useGroupManager(BUILTIN, []));
    act(() =>
      result.current.dispatch({ type: "SET_NEW_GROUP_NAME", name: "Test" }),
    );
  ...
  it("persists custom groups to storage", () => {
    const { result } = renderHook(() => useGroupManager(BUILTIN, []));
    act(() =>
      result.current.dispatch({ type: "ADD_CUSTOM_GROUP", name: "GroupA" }),
    );
    act(() =>
      result.current.dispatch({ type: "ADD_CUSTOM_GROUP", name: "GroupB" }),
    );
  ...
  it("drops persisted group overrides when a connection is removed", () => {
    ...
    expect(JSON.parse(store.get("atlasterm.connectionGroups") ?? "{}")).toEqual(
      { "custom-server": "Staging" },
    );
